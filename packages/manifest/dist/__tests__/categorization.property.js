...
import { categorizeRepository, getWorkspacePath } from '../categorization';
/**
 * Arbitraries for generating test data
 */
const arbitraryTechnology = fc.oneof(fc.constant('TypeScript'), fc.constant('JavaScript'), fc.constant('Python'), fc.constant('Go'), fc.constant('Rust'), fc.constant('Solidity'), fc.constant('Vyper'), fc.constant('Cairo'), fc.stringMatching(/^[A-Z][a-z]+$/));
...
describe('Repository Categorization Consistency', () => {
    it('should deterministically assign repositories to exactly one category', () => {
        fc.assert(fc.property(arbitraryRepositoryMetadata, (metadata) => {
    ...
    it('should assign smart contracts to contract category', () => {
        fc.assert(fc.property(fc.record({
            ...
            description: fc.string({ minLength: 10, maxLength: 200 }),
            technologies: fc.array(fc.oneof(fc.constant('Solidity'), fc.constant('Rust'), fc.constant('Vyper')), { minLength: 1 }),
        }), (metadata) => {
            const category = categorizeRepository(metadata);
    ...
    it('should assign applications to app category', () => {
        fc.assert(fc.property(fc.record({
            ...
            description: fc.string({ minLength: 10, maxLength: 200 }),
            technologies: fc.array(fc.oneof(fc.constant('TypeScript'), fc.constant('React'), fc.constant('Next.js')), { minLength: 1 }),
        }), (metadata) => {
            const category = categorizeRepository(metadata);
    ...
    it('should assign packages/libraries to package category', () => {
        fc.assert(fc.property(fc.record({
            ...
            description: fc.string({ minLength: 10, maxLength: 200 }),
            technologies: fc.array(fc.oneof(fc.constant('TypeScript'), fc.constant('Python'), fc.constant('Go')), { minLength: 1 }),
        }), (metadata) => {
            const category = categorizeRepository(metadata);
...
//# sourceMappingURL=categorization.property.js.map