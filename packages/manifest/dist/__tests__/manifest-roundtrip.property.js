 ...
 */
const arbitraryRepositoryCategory = fc.oneof(fc.constant('app'), fc.constant('package'), fc.constant('contract'), fc.constant('tool'));
const arbitraryRepositoryPriority = fc.oneof(fc.constant('critical'), fc.constant('important'), fc.constant('nice-to-have'));
const arbitraryRepositoryStatus = fc.oneof(fc.constant('pending'), fc.constant('forked'), fc.constant('integrated'), fc.constant('deprecated'));
const arbitraryRepository = fc.record({
    ...
    upstreamSync: fc.record({
        enabled: fc.boolean(),
        lastSync: fc.oneof(fc.constant(null), fc.date({ min: new Date('2020-01-01'), max: new Date() }).map((d) => d.toISOString())),
        branch: fc.stringMatching(/^[a-z0-9-/]+$/),
    }),
});
const arbitraryManifest = fc.record({
    version: fc.tuple(fc.integer({ min: 0, max: 10 }), fc.integer({ min: 0, max: 10 }), fc.integer({ min: 0, max: 10 })).map(([major, minor, patch]) => `${major}.${minor}.${patch}`),
...
describe('Manifest Round-Trip Consistency', () => {
    it('should preserve all fields when serializing and deserializing', () => {
        fc.assert(fc.property(arbitraryManifest, (originalManifest) => {
            // Serialize to JSON
...
//# sourceMappingURL=manifest-roundtrip.property.js.map